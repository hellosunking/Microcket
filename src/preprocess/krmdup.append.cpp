...
void usage( const char * prg ) {
	...
	cerr << "\nThis program is designed to remove the duplicate reads from the FASTQ data using 2 threads."
		 ...
		 << "\n\nThree files will be written in APPEND mode: output.prefix.R1.fq output.prefix.R2.fq output.prefix.log\n\n";
...
void write_batch( ofstream &f1, ofstream &f2, PEREAD *wkr, int nwk ) {
	PEREAD *pend = wkr + nwk;
...
//	cerr << "Prepare files\n";
	...
	string base = kp.outprefix;
	string out = base;
	out += ".read1.fq";
	ofstream fout1( out.c_str(), ios::app );
	out = base;
	out += ".read2.fq";
	ofstream fout2( out.c_str(), ios::app );
...
//		cerr << "Working " << total << "\n";
		if( fqx.eof() ) {	//no need to load data, run with 1 thread is enough
			...
			write_batch( fout1, fout2, wkr, nwk );
			nextBatch = false;
