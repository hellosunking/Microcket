...
void init_kstat_wrbuffer( ktrim_stat &kstat, writeBuffer &writebuffer, unsigned int nthread ) {
	...
	writebuffer.buffer = new char * [ nthread ];
	writebuffer.stored = new unsigned int [ nthread ];

	for(unsigned int i=0; i!=nthread; ++i) {
		writebuffer.buffer[i] = new char[ BUFFER_SIZE_PER_BATCH_READ ];
		writebuffer.stored[i] = 0;
...
//		seed.clear();
		...
		// remove whitespaces in read id
		i = 2;
		while( true ) {
			if( wkr->id1[i]==' ' || wkr->id1[i]=='\t' || wkr->id1[i]=='\n' ) {
				wkr->id1[i] = '\0';
				break;
			}
			++ i;
		}
		writebuffer->stored[tn] += sprintf( writebuffer->buffer[tn]+writebuffer->stored[tn],
											"%s\n%s\n%s\n%s\n%s\n",
											wkr->id1, wkr->seq1, wkr->qual1, wkr->seq2, wkr->qual2 );
	}
...
// prepare memory and file
	...
	#pragma omp parallel
	{
		...
		if( tn == 0 ) {
			...
			init_kstat_wrbuffer( kstat, writebuffer, kp.thread );
	...
	for( unsigned int fileCnt=0; fileCnt!=totalFiles; ++ fileCnt ) {
		...
		if( p[i]=='.' && p[i+1]=='g' && p[i+2]=='z' ) {
			...
			if( gfp1==NULL || gfp2==NULL ) {
				fprintf( stderr, "\033[1;31mError: open gzipped fastq file failed!\033[0m\n" );
				return 104;
			}
		} else {
			fq1 = fopen( p, "rt" );
...
//		fprintf( stderr, "Loading files:\nRead1: %s\nRead2: %s\n", p, q );
		...
		if( loaded == 0 ) {
			cerr << "ERROR START: No reads are loaded from " << R1s[fileCnt] << "!\n";
			exit(1);
		}
		if( loaded != loaded2 ) {
			cerr << "ERROR START: unequal read number for read1 (" << loaded
				 << ") and read2 (" << loaded2 << ")!\n";
			exit(1);
		}
...
//				start = clock();
				...
				if( metEOF ) {
					...
					fwrite( writebuffer.buffer[tn], sizeof(char), writebuffer.stored[tn], stdout );
					nextBatch = false;
...
//				fprintf( stderr, "Thread %d, runtime %.1f\n", tn, duration );
			...
			// update fastq statistics
			// check whether the read-loading is correct
			if( threadLoaded != threadLoaded2 ) {
				cerr << "ERROR: unequal read number for read1 (" << threadLoaded
					 << ") and read2 (" << threadLoaded2 << ")!\n";
				exit(1);
			}
